...
void CharToB1(char sIN[], float nIN[], int amount) {
	...
	for (i = 0; i<amount; i++) {
		...
		if (((sIN[i] - 65) < 6) && ((sIN[i] - 65) >= 0))  nIN[i] = sIN[i] - 53;              //A§A A?A?A±A? A?A§ A A±A?A?A?A®A¤A 
		if (((sIN[i] - 97) < 6) && ((sIN[i] - 97) >= 0))  nIN[i] = sIN[i] - 85;
		if (sIN[i] == 46) nIN[i] = 1;
	}
}
void fallOn2(float nIN[], int amount) {
	int i;													//A?A»A·A?A?A A?A? A?A§ A?A A¦A¤A®A?A® A?A«A?A¬A?A­A?A  A¬A A±A±A?A?A  2
...
int check(float nIN[], int b1, int b2, int amount) {
	...
	erroramount = 0;
	for (i = 0; i<amount; i++) {
		if (nIN[i] == 0) erroramount++;                      //A?A?A«A?A?A?A±A? A«A? A·A?A±A«A® A?1A°A?A·A­A®A© A§A A?A?A±A?A? A? A?A±A? A«A? A?A®A?A?-A?A» A?A?A°A?A?A?A«A?A±A?
		if (nIN[i]>b1 + 1) erroramount++;
	}
	if (b1<2) erroramount++;
	if (b1>16) erroramount++;                              //A«A?A¦A A? A«A? A?1 A? A?2 A? A?A°A?A¤A?A«A Aµ A®A? 2 A¤A® 16
	if (b2<2) erroramount++;
...
int pointCheck(int pointplace, int amount, int pointcount) {
	...
	if (pointplace == amount - 1) {
		result = -1;                      									//A?A®A·A?A  A­A  A?A®A±A«A?A¤A­A?A¬ A¬A?A±A?A?
	}
	if (pointplace == 0) {
		result = -1;                  										//A?A®A·A?A  A­A  A?A?A°A?A®A¬ (A­A?A«A?A?A®A¬) A¬A?A±A?A?
	}
	if (pointplace == 1000) result = 1;								//A­A?A? A?A®A·A?A?
	if ((pointplace < amount - 1) && (pointplace > 0)) result = 0;			//A?A®A·A?A  A­A? A?A® A?A°A A?A¬
	if (pointcount > 1) result = -1;										//A?A®A«A?A?A? 1 A?A®A·A?A?
	return result;
}
...
void reverseArray(int array[], int amount) {
	int i, t;
	for (i = 0; i <= amount / 2; i++) {                                     //A¬A?A­A?A?A? A¬A?A±A?A A¬A? A?A® 2 A?A«A?A¬A?A­A?A  A¬A A±A±A?A?A , A­A A·A?A­A A? A± A?A°A A?A?
		t = array[i];
...
void TenToB2(int nOUT[], double number1[], long long int number2, int b2) {
	...
	while (number2 >= b2) {
		...
		nOUT[i] = left;                     //A?A?A°A?A?A®A¤ A¶A?A«A®A© A·A A±A?A? A·A?A±A«A 
		i++;
		number2 = quot;
	}
	nOUT[i] = number2;
	...
	if (number11 != -2) {
		...
		nOUT[i] = -1;
		pointplace = i;
		do {                                                                 //A?A?A°A?A?A®A¤ A¤A°A®A?A­A®A© A·A A±A?A? A·A?A±A«A 
			i++;
...
void B2ToChar(int nOUT[], char sOUT[], FILE *fout) {
	int i;
	i = 0;
	while (nOUT[i] != -2) {                                                         //A§A A?A?A±A? A? A A±A?A?A?A®A¤ + A?A»A?A®A¤
...
int main() {
	...
	if (fin == NULL)
		...
		fscanf(fin, "%d", &b1);
		fscanf(fin, "%d", &b2);							//A±A·A?A?A»A?A A?A? A¤A A­A­A»A? A?A§ A?A A©A«A 			
