...
struct edge_head* my_edge_head_malloc(struct allocated_edge_head_memory* my_edge_head_memory) { 
	struct edge_head* my_edge_head = &(my_edge_head_memory->memory[my_edge_head_memory->used_memory_cells]);
	my_edge_head_memory->used_memory_cells++;
	return my_edge_head;
};

struct edge_head_list* my_edge_head_list_malloc(struct allocated_edge_head_list_memory* my_edge_head_list_memory) {
	struct edge_head_list* my_edge_head_list = &(my_edge_head_list_memory->memory[my_edge_head_list_memory->used_memory_cells]);
	my_edge_head_list_memory->used_memory_cells++;
	return my_edge_head_list;
};

struct edge_head* create_edge_head(int vertex, struct allocated_edge_head_memory* my_edge_head_memory) {
	struct edge_head* head = my_edge_head_malloc(my_edge_head_memory);
	head->vertex = vertex;
...
struct edge* create_edge( int end, int lenth, struct allocated_edge_memory* my_edge_memory) {
...
struct edge_head_list* insert_edge_head_in_list(struct edge_head_list* list_of_edge_head, struct edge_head* inserted_edge_head, struct allocated_edge_head_list_memory* my_edge_head_list_memory) {
	struct edge_head_list* buff_edge_head_list = list_of_edge_head;
	struct edge_head_list* new_edge_head_list = my_edge_head_list_malloc(my_edge_head_list_memory);
...
int get_number_of_vertices( FILE *fin, bads *is_anything_bad) {
...
void get_start_and_end( FILE *fin, const int number_of_vertices, bads *is_anything_bad, int *start, int *end) {
	int EOF_checker = 0;
...
int get_number_of_edges( FILE *fin, const int number_of_vertices, bads *is_anything_bad) {
...
bads get_edges( FILE *fin, const int number_of_edges, const int number_of_vertices, struct edge_head** edges, struct allocated_edge_memory* my_edge_memory) {
	...
	for (int i = 0; i < number_of_edges; i++) {
		...
		if (first_vertex != second_vertex) {
			...
			put_edge_in_array(edges, create_edge( first_vertex, edge_lenth, my_edge_memory), second_vertex);
			put_edge_in_array(edges, create_edge( second_vertex, edge_lenth, my_edge_memory), first_vertex);
		}
	}
	return not_bad;
}
...
void create_path(struct edge_head_list* list_of_edge_head, bool* is_vertice_checked, struct edge_head**edges, struct allocated_edge_head_list_memory* my_edge_head_list_memory, bool* is_edge_head_in_list) {
	...
	while (processing_edge_head != NULL) {
		...
		while (processing_edge != NULL) {
			if (is_vertice_checked[processing_edge->edge_end]!=true) {
				process_edge(edges, processing_edge, processing_edge_head->vertex);
				if (is_edge_head_in_list[processing_edge->edge_end]==false ) {
					is_edge_head_in_list[processing_edge->edge_end] = true;
					list_of_edge_head = insert_edge_head_in_list(list_of_edge_head, edges[processing_edge->edge_end], my_edge_head_list_memory);
				}
			}
...
void print_path( struct edge_head** edges,  int ending_vertex, FILE* fout) {
...
int main() {
	...
	struct edge_head* edge_head_memory = malloc(number_of_vertices* (sizeof(struct edge_head)));
	struct allocated_edge_head_memory* my_edge_head_memory = malloc(sizeof(struct allocated_edge_head_memory));
	my_edge_head_memory->memory = edge_head_memory;
	my_edge_head_memory->used_memory_cells = 0;

	struct edge_head**edges = malloc((number_of_vertices + 1) * sizeof(struct edge_head*));
	for (int i = 1; i < number_of_vertices + 1; i++) {
		edges[i] = create_edge_head(i,my_edge_head_memory);
	}
	...
	bool* is_edge_head_in_list = malloc((number_of_vertices + 1) * sizeof(bool));
	for (int i = 1; i < number_of_vertices + 1; i++) {
		is_edge_head_in_list[i] = false;
	}
	struct edge_head_list* edge_head_list_memory = malloc(number_of_vertices* (sizeof(struct edge_head_list)));
	struct allocated_edge_head_list_memory* my_edge_head_list_memory = malloc(sizeof(struct allocated_edge_head_list_memory));
	my_edge_head_list_memory->memory = edge_head_list_memory;
	my_edge_head_list_memory->used_memory_cells = 0;

	struct edge_head_list*list_of_edge_head = my_edge_head_list_malloc( my_edge_head_list_memory);
	list_of_edge_head->next = NULL;
	list_of_edge_head->head = edges[starting_vertex];
	is_edge_head_in_list[starting_vertex] = true;
	create_path(list_of_edge_head, is_vertice_checked, edges,my_edge_head_list_memory, is_edge_head_in_list);
	...
	free(edges);
	free(edge_memory);
	free(edge_head_memory);
	free(edge_head_list_memory);
	free(is_vertice_checked);
	free(is_edge_head_in_list);
	free(my_edge_head_list_memory);
	free(my_edge_head_memory);
	free(my_edge_memory);
